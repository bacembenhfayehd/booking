...
export default function PhotosUploader({addedPhotos,onChange}) {
    ...
    async function addPhotoByLink(ev) {
        ...
        // Mettre à jour l'état addedPhotos en ajoutant le nouveau nom de fichier avec le chemin relatif
        onChange(prev => {
            return [...prev, `uploads/${filename}`]; // Assurez-vous que le chemin 'uploads/' correspond à celui utilisé par votre serveur pour stocker les images
        });
        // Mettre à jour l'état photoLink pour effacer le champ de saisie
        setPhotoLink('');
    }
    ...
    function uploadPhoto(ev){
        ...
        for (let i = 0; i < files.length; i++){
            data.append('photos', files[i]); // Ajoute chaque fichier à FormData
        }
        ...
        axios.post('/upload', data, {
            ...
            onChange(prev => [...prev, ...filenames]); // Ajoute les noms de fichiers à l'état
        }).catch(error => {
